...
- Surface hierarchy is intentional: `muted` is neutral `/4`; `accent`, `secondary`, and `sidebar-accent` are neutral `/8`; `secondary-hover` is neutral `/16`. Accent communicates interactive context without becoming a second primary surface.
...
Every API Reference table uses the exact header `| Prop | Type | Default |`. Use `-` when there is no default; do not use alternate headers or description columns.
