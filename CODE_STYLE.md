...
- Focus: `outline-hidden` + `border-ring/64 ring-2 ring-ring/24`. Base `border border-transparent` when no structural border (fields keep `border-input` until focused). Solid `bg-primary`: opaque `border-background` + same ring. No `outline-none` unless a documented a11y exception must suppress focus in forced-colors. No outline utils on static/decorative elements. Command search field is the only exception: no focus ring and no border shift (`focus-within:border-input focus-within:ring-0`). The highlighted list item is the focus cue.
...
- Drawer exception: local transitions over `--drawer-translate-x/y`, `--drawer-swipe-progress`, `--drawer-swipe-strength`, `--drawer-rest-height`, `--nested-swipe-progress`. Panel and nested stack `duration-[450ms] ease-[cubic-bezier(0.32,0.72,0,1)]` (no fade); backdrop `duration-[450ms] ease-out` fade. Disable transitions while swiping; close duration `calc(var(--drawer-swipe-strength) * 400ms)`. No Drawer CSS outside `drawer.tsx`.
- Ark-positioned overlays: `origin-(--transform-origin)`, 98% scale, fade, placement-aware travel, local overlay classes. Centered dialogs / coordinate-positioned panels: `origin-center`. Ban `scale(0)`, `ease-in`, `ease-linear`, `transition-all`, arbitrary easing — except Sidebar shadcn geometry (`duration-200 ease-linear` for width/inset/margin/opacity) and the Drawer sheet curve above.
