...
- Drawer exception: local transitions over `--drawer-translate-x/y`, `--drawer-swipe-progress`, `--nested-swipe-progress`, `--drawer-swipe-strength`. Panel and nested stack `duration-[450ms] ease-[cubic-bezier(0.32,0.72,0,1)]` (no fade); backdrop `duration-[450ms] ease-out` fade. Disable transitions while swiping; close duration `calc(var(--drawer-swipe-strength) * 400ms)`. No Drawer CSS outside `drawer.tsx`.
