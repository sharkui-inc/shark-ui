...
export const CalendarThumb = (props: ThumbCardProps) => (
  <ThumbCard {...props}>
    <div className="relative flex w-48 flex-col gap-2 rounded-lg border border-input bg-muted p-3 shadow-sm/4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-1 items-center justify-center gap-2">
          ...
          <div className="h-2 w-3/4 rounded-lg bg-muted-foreground/24" />
        </div>
        <div className="flex size-5 items-center justify-center rounded-lg border border-input bg-muted">
          <ChevronLeftIcon
            aria-hidden="true"
      ...
      <div className="grid grid-cols-5 place-content-center gap-1.5">
        {Array.from({ length: 5 }, (_, i) => `day-${i}`).map((key) => (
          <div className="h-1.5 rounded-lg bg-muted-foreground/24" key={key} />
        ))}
      </div>
