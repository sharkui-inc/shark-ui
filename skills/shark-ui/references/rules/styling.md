...
- **Base variant contracts.** Four primitives own shared `tv()` variants. Other components apply those exports on their own Ark parts so a change in the base file updates the family. Do not redeclare `h-8` / `min-h-8`, field chrome, list-row metrics, or modal overlay/content classes. Composition-only parts (Field, Separator, Scroll Area, Spinner, Badge) are not bases: consumers render the component, they do not copy its `tv()`.
  - **Button** (`buttonControlVariants`, `buttonVariants`): pressable control chrome. Toggle, Tabs, Segment Group, Navigation Menu, Sidebar, Editable, Pagination.
  - **Input** (`inputVariants`, `inputItemVariants`): field chrome (`h-8` / `h-7` / `h-9`, border, ring) and selection-row typography. Clipboard, Combobox, Select/Listbox rows (`inputItemVariants`).
  - **Menu** (`menuItemControlVariants`, `menuListVariants`, and related list tokens): list-row density. See the list-items bullet below.
  - **Dialog** (`dialogOverlayVariants`, `dialogContentVariants`): modal surface. Command uses `dialogContentVariants`; Tour uses `dialogOverlayVariants`; Sheet and Alert Dialog re-export Dialog parts.
- **List items** (Menu, ContextMenu, Select, Combobox, Autocomplete, ModelSelector, NavigationMenu, PromptInputPopover, Command, Listbox) use Menu variants as the density contract. `menuItemControlVariants` (`min-h-8`, `rounded-lg`, `gap-2`, item padding) owns row size — including Listbox, which is not a floating overlay. Floating list overlays also use `menuListVariants` (`p-1`) for the gutter between the popover border and item highlights. Group labels use `menuGroupLabelVariants`, compact empty states use `menuEmptyVariants`, and separators use `menuSeparatorVariants` (`my-1`). Do not redeclare `min-h-8`, item padding, or extra `gap-1` between rows. Do not use `rounded-xl` on list rows. Put `p-0` on the shell when a `ScrollArea` wraps the list; do not use `p-1.5` or `p-2` on these contents. Selection rows compose `menuItemControlVariants` with `inputItemVariants` (typography only). Content popovers (HoverCard, DatePicker, ColorPicker, default Popover `--space`) are not list menus and keep their own padding.
...
11. List items use Menu `menuItemControlVariants` (`min-h-8`, `rounded-lg`, not `rounded-xl`); overlays also use `menuListVariants` (`p-1`), `menuGroupLabelVariants`, `menuEmptyVariants`, and `menuSeparatorVariants`?
12. New controls import the base `tv()` (Button / Input / Menu / Dialog) instead of redeclaring `h-8`, `min-h-8`, field chrome, or overlay/content classes?
13. Duplicated layout already provided by the primitive?
14. Icon semantics and `aria-hidden` correct?
15. `data-slot` / compound part structure preserved when extending styles?
