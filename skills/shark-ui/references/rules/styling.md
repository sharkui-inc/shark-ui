...
- **Reduced motion (CSS `prefers-reduced-motion`, not the Motion library):** use Tailwind’s `motion-reduce:` variant (`@media (prefers-reduced-motion: reduce)`) — not Motion / Framer Motion. Add `motion-reduce:transition-none!` and/or `motion-reduce:animate-none!` **once** at the end of each built-in class list in `cn()` / `tv()` — **after** component defaults, **before** the `className` argument (keep `className` last; important `!`). Example: `cn("…defaults", "motion-reduce:transition-none!", className)`. For `tv()`, put those utilities at the end of `base` (e.g. `buttonVariants`). When merging variants with `cn()`, use `cn(variants({ ... }), className)` if they are already in `tv()`, otherwise `cn(variants({ ... }), "motion-reduce:…!", className)`. Use nested selectors when needed (e.g. `[a]:motion-reduce:transition-none!`, `[&>span]:motion-reduce:animate-none!`).
