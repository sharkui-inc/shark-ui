...
- **Reduced motion (`prefers-reduced-motion`):** add `motion-reduce:transition-none!` and/or `motion-reduce:animate-none!` **once** at the end of each built-in class list in `cn()` / `tv()` — **after** component defaults, **before** the `className` argument (keep `className` last; important `!`). Example: `cn("…defaults", "motion-reduce:transition-none!", className)`. Use nested selectors when needed (e.g. `[a]:motion-reduce:transition-none!`, `[&>span]:motion-reduce:animate-none!`).
