---
title: Accordion
description: Expand and collapse stacked sections.
seo:
  description: Expand and collapse stacked sections of content.
links:
  doc: https://ark-ui.com/docs/components/accordion
  api: https://ark-ui.com/docs/components/accordion#api-reference
---

import manifest from "@/registry/manifest/accordion";

<ComponentPreview componentName="accordion" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/accordion
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Ensure your CSS entry imports `tw-animate-css`.</Step>

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/accordion.tsx" src="/accordion.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Anatomy

```text
Accordion
└── AccordionItem
    ├── AccordionTrigger
    └── AccordionContent
```

## Usage

```tsx
import { 
  Accordion, 
  AccordionItem, 
  AccordionTrigger, 
  AccordionContent 
} from "@/components/ui/accordion";
```

```tsx
<Accordion>
  <AccordionItem value="item-1">
    <AccordionTrigger>
      Is it accessible?
    </AccordionTrigger>
    <AccordionContent>
      Yes. It adheres to the WAI-ARIA design pattern.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```

## Controlled

Use the `value` and `onValueChange` props to control the accordion.

<ComponentPreview componentName="accordion" fileName="example-controlled" />

## States

### Disabled

<ComponentPreview componentName="accordion" fileName="example-disabled" />

## Examples

### Non-collapsible

Set the `collapsible` prop to `false` to disable the collapsible behavior.

<ComponentPreview componentName="accordion" fileName="example-non-collapsible" />

### Multiple

Use the `multiple` prop to allow multiple open items.

<ComponentPreview componentName="accordion" fileName="example-multiple" />

### Card

Wrap the `Accordion` in a `Card` component.

<ComponentPreview componentName="accordion" fileName="example-card" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="accordion" />

## API Reference

### Accordion

Root component.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `string[]` | `-` |
| `defaultValue` | `string[]` | `-` |
| `collapsible` | `boolean` | `true` |
| `multiple` | `boolean` | `false` |
| `onValueChange` | `ValueChangeDetails` | `-` |
| `lazyMount` | `boolean` | `true` |
| `unmountOnExit` | `boolean` | `true` |
| `hideMode` | `"display-none" \| "activity"` | `"display-none"` |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### AccordionItem

Single accordion item.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `string` | **required** |
| `disabled` | `boolean` | `false` |
| `className` | `string` | `-` |

### AccordionTrigger

Toggles the item open/closed on click.

| Prop | Type | Default |
| --- | --- | --- |
| `disabled` | `boolean` | `false` |
| `className` | `string` | `-` |

### AccordionContent

Holds the collapsible content for the item.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### AccordionRootProvider

Provides a Accordion API created with `useAccordion` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseAccordionReturn` | **required** |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/accordion#api-reference).

## Related

- [Collapsible](/docs/components/collapsible) - show/hide content block
- [Tabs](/docs/components/tabs) - tabbed panel navigation
- [Sidebar](/docs/components/sidebar) - responsive app sidebar
