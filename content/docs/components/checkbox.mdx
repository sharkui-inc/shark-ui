---
title: Checkbox
description: Checkbox for multiple selections.
seo:
  description: Checkbox for multi-select forms, filters, and agreements.
links:
  doc: https://ark-ui.com/docs/components/checkbox
  api: https://ark-ui.com/docs/components/checkbox#api-reference
---

import manifest from "@/registry/manifest/checkbox";

<ComponentPreview componentName="checkbox" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/checkbox
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/checkbox.tsx" src="/checkbox.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { Checkbox } from "@/components/ui/checkbox";
```

```tsx showLineNumbers
<Checkbox />
```

## Controlled

Use the `checked` and `onCheckedChange` props to control the checkbox.

<ComponentPreview componentName="checkbox" fileName="example-controlled" />

## States

### Invalid

Use the `invalid` prop to indicate an invalid state.

<ComponentPreview componentName="checkbox" fileName="example-invalid" />

### Disabled

Use the `disabled` prop to disable the checkbox.

<ComponentPreview componentName="checkbox" fileName="example-disabled" />

## Examples

### With Description

<ComponentPreview componentName="checkbox" fileName="example-with-description" />

### Indeterminate

<ComponentPreview componentName="checkbox" fileName="example-indeterminate" />

### Group

<ComponentPreview componentName="checkbox" fileName="example-checkbox-group" />

### Max selected

Use `maxSelectedValues` on `CheckboxGroup` to cap how many boxes can be checked.

<ComponentPreview componentName="checkbox" fileName="example-max-selected" />

### Card Style

<ComponentPreview componentName="checkbox" fileName="example-card" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="checkbox" />

## API Reference

### Checkbox

Checkbox control for boolean or multi-select state.

| Prop | Type | Default |
| --- | --- | --- |
| `checked` | `boolean \| 'indeterminate'` | `-` |
| `defaultChecked` | `boolean` | `false` |
| `disabled` | `boolean` | `false` |
| `checked` | `boolean \| 'indeterminate'` | `-` |
| `onCheckedChange` | `({ checked }: CheckedChangeDetails) => void` | `-` |
| `className` | `string` | `-` |

### CheckboxGroup

Groups related checkboxes with consistent layout.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `string[]` | `-` |
| `defaultValue` | `string[]` | `[]` |
| `onValueChange` | `(details: ValueChangeDetails) => void` | `-` |
| `maxSelectedValues` | `number` | `-` |
| `className` | `string` | `-` |

### CheckboxRootProvider

Provides a Checkbox API created with `useCheckbox` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseCheckboxReturn` | **required** |

### CheckboxIndicator

Public CheckboxIndicator component. It forwards the underlying primitive props.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | - |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/checkbox#api-reference).

## Related

- [Radio Group](/docs/components/radio-group) - single-select radios
- [Switch](/docs/components/switch) - binary on/off switch
- [Toggle](/docs/components/toggle) - two-state toggle button
