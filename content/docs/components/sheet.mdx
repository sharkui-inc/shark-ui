---
title: Sheet  
description: Panel that slides in from the edge.
seo:
  description: Panel that slides in from the edge for forms and filters.
links:
  doc: https://ark-ui.com/docs/components/dialog
  api: https://ark-ui.com/docs/components/dialog#api-reference
---

import manifest from "@/registry/manifest/sheet";

<ComponentPreview componentName="sheet" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/sheet
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>This component depends on [Button](/docs/components/button) and [Dialog](/docs/components/dialog). Install them first if you haven't already.</Step>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/sheet.tsx" src="/sheet.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Anatomy

```text
Sheet
├── SheetTrigger
└── SheetContent
    ├── SheetHeader
    │   ├── SheetTitle
    │   └── SheetDescription
    ├── SheetBody
    └── SheetFooter
        └── SheetClose
```

## Usage

```tsx
import { 
  Sheet, 
  SheetContent, 
  SheetDescription, 
  SheetHeader, 
  SheetTitle, 
  SheetTrigger 
} from "@/components/ui/sheet";
```

```tsx
<Sheet>
  <SheetTrigger>Open</SheetTrigger>
  <SheetContent>
    <SheetHeader>
      <SheetTitle>Sheet Title</SheetTitle>     
      <SheetDescription>Sheet Description</SheetDescription>
    </SheetHeader>
  </SheetContent>
</Sheet>
```

## Controlled

Use `open` and `onOpenChange` on the root to control the sheet state.

<ComponentPreview componentName="sheet" fileName="example-controlled" />

## Sides

Control which side the sheet slides in from using the `side` prop.

<ComponentPreview componentName="sheet" fileName="example-sides" />

## Variants

### Default

<ComponentPreview componentName="sheet" fileName="example-variant-default" />

### Inset

Use `variant="inset"` to give the sheet a 12px inset on mobile and 16px from `sm` upward.

<ComponentPreview componentName="sheet" fileName="example-inset" />

## Examples

### Scrollable

Use `SheetBody` to make the content area scrollable while keeping header and footer fixed.

<ComponentPreview componentName="sheet" fileName="example-scroll-area" />

### No Close Button

Use the `showCloseButton={false}` prop to hide the close button in the top right corner.

<ComponentPreview componentName="sheet" fileName="example-no-close-button" />

### Close behavior

Use `closeOnInteractOutside` and `closeOnEscape` props to prevent closing on outside click and escape.

<ComponentPreview componentName="sheet" fileName="example-close-behavior" />

### Non-Modal

Use `modal={false}` to allow interaction with elements outside the sheet.

<ComponentPreview componentName="sheet" fileName="example-non-modal" />

### Multiple triggers

`value` on each `SheetTrigger`. `onTriggerValueChange` tracks which button opened the sheet.

<ComponentPreview componentName="sheet" fileName="example-multiple-triggers" />

### Custom spacing

The `[--space:--spacing("value")]` controls the internal spacing.

<ComponentPreview componentName="sheet" fileName="example-custom-spacing" />

> You can use breakpoint utilities to change the internal spacing at different screen sizes.

```css
  md:[--space:--spacing(6)] lg:[--space:--spacing(8)]
```

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="sheet" />

## API Reference

### Sheet

Root component.

| Prop | Type | Default |
| --- | --- | --- |
| `open` | `boolean` | `-` |
| `defaultOpen` | `boolean` | `false` |
| `onOpenChange` | `({ open }: OpenChangeDetails) => void` | `-` |
| `onTriggerValueChange` | `(details: TriggerValueChangeDetails) => void` | `-` |
| `modal` | `boolean` | `true` |
| `closeOnEscape` | `boolean` | `true` |
| `closeOnInteractOutside` | `boolean` | `true` |
| `initialFocusEl` | `() => MaybeElement` | `-` |
| `finalFocusEl` | `() => MaybeElement` | `-` |
| `lazyMount` | `boolean` | `true` |
| `unmountOnExit` | `boolean` | `true` |
| `hideMode` | `"display-none" \| "activity"` | `"display-none"` |
| `onEscapeKeyDown` | `(event: KeyboardEvent) => void` | `-` |
| `onInteractOutside` | `(event: InteractOutsideEvent) => void` | `-` |
| `className` | `string` | `-` |

### SheetTrigger

Opens the sheet on click. Use `asChild` for custom triggers.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `value` | `string` | `-` |
| `className` | `string` | `-` |

### SheetContent

Holds the sheet panel. Slides in from the configured edge. `placement="left"` maps to the inline-start edge (left in LTR, right in RTL); `placement="right"` maps to inline-end.

| Prop | Type | Default |
| --- | --- | --- |
| `placement` | `"right" \| "left" \| "top" \| "bottom"` | `"right"` |
| `variant` | `"default" \| "inset"` | `"default"` |
| `showCloseButton` | `boolean` | `true` |
| `className` | `string` | `-` |

| Prop | Type | Default |
| --- | --- | --- |
| `--space` | `string` | `--spacing(6)` |

### SheetHeader

Header area for title and description.

| Prop | Type | Default |
| --- | --- | --- |
| `title` | `string` | `-` |
| `description` | `string` | `-` |
| `className` | `string` | `-` |

### SheetBody

Scrollable body content.

| Prop | Type | Default |
| --- | --- | --- |
| `scrollFade` | `boolean` | `true` |
| `className` | `string` | `-` |

### SheetFooter

Footer area for action buttons. Same pattern as `DialogFooter`: primary action first in the DOM; layout classes reverse the visual order (Cancel left / Confirm right).

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |

### SheetTitle

Accessible title. Announced to screen readers.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | `-` |

### SheetDescription

Accessible description. Announced to screen readers.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | `-` |

### SheetClose

Closes the sheet on click.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | `-` |

### SheetOverlay

Dimmed overlay behind the sheet.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |

### SheetPositioner

Positioning wrapper for sheet content. Used internally; expose for custom layouts.

| Prop | Type | Default |
| --- | --- | --- |
| `side` | `"right" \| "left" \| "top" \| "bottom"` | `-` |
| `variant` | `"default" \| "inset"` | `"default"` |
| `className` | `string` | `-` |

### SheetRootProvider

Provides a Sheet API created with `useSheet` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseSheetReturn` | **required** |
| `modal` | `boolean` | `true` |

`modal` gates the overlay the same way as on `Sheet` / `Dialog`. Pass the same value you used in `useSheet` when it is not the default.

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/dialog#api-reference).
