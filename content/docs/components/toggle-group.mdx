...
Use the `value` and `onValueChange` props to control the toggle group.
...
The `pill` prop creates a fully rounded segmented toggle group.
...
<RTLComponentPreview componentName="toggle-group" />
...
Provides a ToggleGroup API created with `useToggleGroup` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseToggleGroupReturn` | `required` |
| `pill` | `boolean` | `false` |
| `size` | `"sm" \| "md" \| "lg"` | `"md"` |
| `spacing` | `number` | `0` |
| `variant` | `"ghost" \| "outline"` | `"ghost"` |

Pass the same styling props you would use on `ToggleGroup` so `ToggleGroupItem` receives them under the headless path.
