---
title: Toggle
description: Two-state pressed/unpressed button.
seo:
  description: Pressed/unpressed button for toolbars and filters.
links:
  doc: https://ark-ui.com/docs/components/toggle
  api: https://ark-ui.com/docs/components/toggle#api-reference
---

import manifest from "@/registry/manifest/toggle";

<ComponentPreview componentName="toggle" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/toggle
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>This component depends on [Button](/docs/components/button). Install it first if you haven't already.</Step>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/toggle.tsx" src="/toggle.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { Toggle } from "@/components/ui/toggle";
```

```tsx
<Toggle>Toggle</Toggle>
```

## Controlled

Use the `pressed` and `onPressedChange` props to control the toggle.

<ComponentPreview componentName="toggle" fileName="example-controlled" />

## States

### Disabled

Use the `disabled` prop to disable the toggle.

<ComponentPreview componentName="toggle" fileName="example-disabled" />


## Variants

### Default

<ComponentPreview componentName="toggle" fileName="example-default" />

### Outline

<ComponentPreview componentName="toggle" fileName="example-outline" />

## Sizes

### Small

<ComponentPreview componentName="toggle" fileName="example-size-sm" />

### Medium

<ComponentPreview componentName="toggle" fileName="example-size-md" />

### Large

<ComponentPreview componentName="toggle" fileName="example-size-lg" />

## Pill

The `pill` prop creates fully rounded toggles.

<ComponentPreview componentName="toggle" fileName="example-pill" />

## Examples

### Icon Group

Use multiple toggles with icons for toolbar-style controls.

<ComponentPreview componentName="toggle" fileName="example-icon-group" />

### Stateful label

`ToggleIndicator` renders its children when pressed and `fallback` when not.

<ComponentPreview componentName="toggle" fileName="example-indicator" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="toggle" />

## API Reference

### Toggle

Toggle button for on/off or selected state. Use `asChild` for custom elements.

| Prop | Type | Default |
| --- | --- | --- |
| `variant` | `"outline" \| "ghost"` | `"ghost"` |
| `size` | `"sm" \| "md" \| "lg"` | `"md"` |
| `pill` | `boolean` | `false` |
| `pressed` | `boolean` | `-` |
| `defaultPressed` | `boolean` | `false` |
| `disabled` | `boolean` | `false` |
| `onPressedChange` | `(details: PressedChangeDetails) => void` | `-` |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### ToggleIndicator

Renders its children when the toggle is pressed and `fallback` when it is not.

| Prop | Type | Default |
| --- | --- | --- |
| `fallback` | `ReactNode` | `-` |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/toggle#api-reference).
