...
seo:
  description: Numeric field with steppers for discrete increments.
links:
  doc: https://ark-ui.com/docs/components/number-input
  api: https://ark-ui.com/docs/components/number-input#api-reference
---

import manifest from "@/registry/manifest/number-input";
...
Hold `Shift` for `largeStep` and `Alt` for `smallStep`.
...
<RTLComponentPreview componentName="number-input" />
...
Provides a NumberInput API created with `useNumberInput` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseNumberInputReturn` | **required** |
