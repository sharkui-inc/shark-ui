---
title: Action Bar
description: Bulk actions for selected items.
seo:
  description: Bulk action bar for selected rows in lists and tables.
---

import manifest from "@/registry/manifest/action-bar";

<ComponentPreview componentName="action-bar" />

## Installation

<CodeTabs>
<TabsList>
    <TabsTrigger value="cli">CLI</TabsTrigger>
    <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>
<TabsContent value="cli">

```bash
npx shadcn@latest add @shark/action-bar
```

</TabsContent>
<TabsContent value="manual">

<Steps>
<Step>This component depends on [Badge](/docs/components/badge) and [Separator](/docs/components/separator). Install them first if you haven't already.</Step>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

  <ComponentSource title="components/ui/action-bar.tsx" src="/action-bar.tsx" />

<Step>Update the import paths to match your project setup.</Step>
</Steps>

</TabsContent>
</CodeTabs>

## Anatomy

```text
ActionBar
├── ActionBarTrigger
└── ActionBarContent
    ├── ActionBarValue
    ├── ActionBarSeparator
    ├── ActionBarBody
    │   └── Button
    ├── ActionBarSeparator
    └── ActionBarClose
```

## Usage

```tsx
import {
  ArchiveIcon,
  DownloadIcon,
  PencilIcon,
  Trash2Icon,
  XIcon,
} from "lucide-react";
import {
  ActionBar,
  ActionBarBody,
  ActionBarClose,
  ActionBarContent,
  ActionBarSeparator,
  ActionBarTrigger,
  ActionBarValue,
} from "@/components/ui/action-bar";
import { Button } from "@/components/ui/button";
```

```tsx
<ActionBar>
  <ActionBarTrigger asChild>
    <Button variant="outline">Open</Button>
  </ActionBarTrigger>
  <ActionBarContent aria-label="Bulk actions">
    <ActionBarValue count={3} />
    <ActionBarSeparator />
    <ActionBarBody>
      <Button variant="ghost">
        <PencilIcon aria-hidden="true" data-icon="inline-start" />
        <span className="max-sm:sr-only">Edit</span>
      </Button>
      <ActionBarSeparator />
      <Button variant="destructive">
        <Trash2Icon aria-hidden="true" data-icon="inline-start" />
        <span className="max-sm:sr-only">Delete</span>
      </Button>
    </ActionBarBody>
    <ActionBarSeparator />
    <ActionBarClose asChild>
      <Button size="icon-md" variant="ghost">
        <XIcon aria-hidden="true" />
      </Button>
    </ActionBarClose>
  </ActionBarContent>
</ActionBar>
```

## Controlled

<ComponentPreview componentName="action-bar" fileName="example-controlled" />

## Examples

### Positioning

Use the `positioning.placement` prop to position the action bar.

<ComponentPreview componentName="action-bar" fileName="example-positioning" />

### Gutter

Use `positioning.gutter` to control the distance from the bottom edge.

<ComponentPreview componentName="action-bar" fileName="example-gutter" />

### Close Trigger

<ComponentPreview componentName="action-bar" fileName="example-close-trigger" />

### With Dialog

<ComponentPreview componentName="action-bar" fileName="example-dialog" />

### With Menu

<ComponentPreview componentName="action-bar" fileName="example-menu" />

### Table

<ComponentPreview componentName="action-bar" fileName="example-table" />

### Custom Spacing

The `[--space:--spacing("value")]` controls the internal spacing.

<ComponentPreview componentName="action-bar" fileName="example-custom-spacing" />

> You can use breakpoint utilities to change the internal spacing at different screen sizes.

```css
md:[--space:--spacing(6)] lg:[--space:--spacing(8)]
```

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="action-bar" />

## API Reference

### ActionBar

Root component. Provides open state and positioning to descendants. Escape and
outside interaction do not close the ActionBar, and it does not move focus
automatically.

| Prop | Type | Default |
| --- | --- | --- |
| `open` | `boolean` | - |
| `defaultOpen` | `boolean` | `false` |
| `positioning` | `ActionBarPositioning` | `{ placement: "bottom", gutter: "16px" }` |
| `lazyMount` | `boolean` | `true` |
| `unmountOnExit` | `boolean` | `true` |
| `onOpenChange` | `({ open }: OpenChangeDetails) => void` | - |

### ActionBarContent

Non-modal action surface.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | - |
| `aria-label` | `string` | `"Bulk actions"` |
| `aria-labelledby` | `string` | - |
| `asChild` | `boolean` | - |

| Prop | Type | Default |
| --- | --- | --- |
| `--space` | `string` | `--spacing(2)` |

### ActionBarTrigger

Button that toggles the action bar.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | - |
| `asChild` | `boolean` | - |

### ActionBarValue

Displays the selection count.

| Prop | Type | Default |
| --- | --- | --- |
| `count` | `number` | - |
| `label` | `string` | - |
| `className` | `string` | - |
| `asChild` | `boolean` | - |

### ActionBarSeparator

Vertical rule. Renders a [Separator](/docs/components/separator).

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | - |
| `asChild` | `boolean` | - |

### ActionBarBody

Flex row for primary actions.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | - |
| `asChild` | `boolean` | - |

### ActionBarClose

Closes the action bar.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | - |
| `asChild` | `boolean` | - |
