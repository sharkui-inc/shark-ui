...
Put a `Sidebar` inside `DialogContent` with a fixed height so switching pages does not resize the panel. Do not wrap this layout in `DialogBody` (that part is a single scroll area). Override `SidebarProvider`’s `min-h-svh` with `min-h-0`, and use `collapsible="none"` so the sidebar does not open a nested sheet.
