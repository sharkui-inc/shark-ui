---
title: Clipboard  
description: Copy text to the clipboard.
seo:
  description: Copy a value to the clipboard with copied feedback.
links:
  doc: https://ark-ui.com/docs/components/clipboard 
  api: https://ark-ui.com/docs/components/clipboard#api-reference
---

import manifest from "@/registry/manifest/clipboard";

<ComponentPreview componentName="clipboard" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/clipboard
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>This component depends on [Input](/docs/components/input). Install it first if you haven't already.</Step>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/clipboard.tsx" src="/clipboard.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Anatomy

```text
Clipboard
├── ClipboardTrigger
│   └── ClipboardIndicator
├── ClipboardInput
└── ClipboardValue
```

## Usage

```tsx
import { 
  Clipboard, 
  ClipboardTrigger, 
  ClipboardIndicator 
} from "@/components/ui/clipboard";
```

```tsx
<Clipboard value="https://x.com/vinihvc">
  <ClipboardTrigger asChild>
    <Button size="icon-md">
      <ClipboardIndicator />
    </Button>
  </ClipboardTrigger>
</Clipboard>
```

## Controlled

<ComponentPreview componentName="clipboard" fileName="example-controlled" />

## Examples

### Icon Only

<ComponentPreview componentName="clipboard" fileName="example-icon-only" />

### Different Icons

<ComponentPreview componentName="clipboard" fileName="example-different-icon" />

### Value Text

Use `ClipboardValue` component to display the value as text instead of an input field.

<ComponentPreview componentName="clipboard" fileName="example-value-text" />

### Custom Timeout

Use the `timeout` prop on `Clipboard` to customize how long the copied state is shown.

<ComponentPreview componentName="clipboard" fileName="example-custom-timeout" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="clipboard" />

## API Reference

### Clipboard

Root wrapper. Manages copy state and value.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `string` | `required` |
| `timeout` | `number` | `2000` |
| `className` | `string` | `-` |

### ClipboardTrigger

Triggers copy to clipboard on click.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### ClipboardInput

Input showing the value to copy. Often hidden or read-only.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### ClipboardIndicator

Icon that changes based on copy state.

| Prop | Type | Default |
| --- | --- | --- |
| `copied` | `ReactNode` | `CheckIcon` |
| `className` | `string` | `-` |

### ClipboardValue

Text display of the value. Use when input is not needed.

| Prop | Type | Default |
| --- | --- | --- |
| `size` | `"xs" \| "sm" \| "md" \| "lg" \| "xl"` | `"md"` |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### ClipboardRootProvider

Provides a Clipboard API created with `useClipboard` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseClipboardReturn` | `required` |

### ClipboardContext

Render-prop access to the surrounding component API.

| Prop | Type | Default |
| --- | --- | --- |
| `children` | `(api: unknown) => React.ReactNode` | `required` |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/clipboard#api-reference).
