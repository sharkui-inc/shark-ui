---
title: Breadcrumb
description: Trail of links in a page hierarchy.
seo:
  description: Trail of links that shows where you are in a page hierarchy.
---

import manifest from "@/registry/manifest/breadcrumb";

<ComponentPreview componentName="breadcrumb" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```bash
npx shadcn@latest add @shark/breadcrumb
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/breadcrumb.tsx" src="/breadcrumb.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Anatomy

```text
Breadcrumb
└── BreadcrumbList
    └── BreadcrumbItem
        ├── BreadcrumbLink
        ├── BreadcrumbPage
        ├── BreadcrumbSeparator
        └── BreadcrumbEllipsis
```

## Usage

```tsx
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
  BreadcrumbList,
  BreadcrumbPage,
  BreadcrumbSeparator,
} from "@/components/ui/breadcrumb";
```

```tsx
<Breadcrumb>
  <BreadcrumbList>
    <BreadcrumbItem>
      <BreadcrumbLink href="/">Home</BreadcrumbLink>
    </BreadcrumbItem>
    <BreadcrumbSeparator />
    <BreadcrumbItem>
      <BreadcrumbLink href="/components">Components</BreadcrumbLink>
    </BreadcrumbItem>
    <BreadcrumbSeparator />
    <BreadcrumbItem>
      <BreadcrumbPage>Breadcrumb</BreadcrumbPage>
    </BreadcrumbItem>
  </BreadcrumbList>
</Breadcrumb>
```

## Examples

### With custom separator

<ComponentPreview componentName="breadcrumb" fileName="example-custom-separator" />

### With links

<ComponentPreview componentName="breadcrumb" fileName="example-with-link" />

### Collapsed

<ComponentPreview componentName="breadcrumb" fileName="example-collapsed" />

### With menu

<ComponentPreview componentName="breadcrumb" fileName="example-with-menu" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="breadcrumb" />

## API Reference

### Breadcrumb

Navigation landmark for the breadcrumb trail.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### BreadcrumbList

Ordered list of breadcrumb segments.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### BreadcrumbItem

Wraps a single breadcrumb link or page.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### BreadcrumbLink

Link to a parent page in the hierarchy.

| Prop | Type | Default |
| --- | --- | --- |
| `href` | `string` | `-` |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### BreadcrumbPage

Current page. Not a link; marked with `aria-current="page"` for accessibility.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### BreadcrumbSeparator

Visual separator between segments.

| Prop | Type | Default |
| --- | --- | --- |
| `children` | `React.ReactNode` | `ChevronRight` |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### BreadcrumbEllipsis

Shows when middle segments are collapsed.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

## Related

- [Pagination](/docs/components/pagination) - paged list navigation
- [Sidebar](/docs/components/sidebar) - responsive app sidebar
- [Tabs](/docs/components/tabs) - tabbed panel navigation
