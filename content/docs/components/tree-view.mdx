---
title: Tree View
description: Hierarchical data in a tree.
seo:
  description: Expandable tree for files, folders, and nested data.
links:
  doc: https://ark-ui.com/docs/components/tree-view
  api: https://ark-ui.com/docs/components/tree-view#api-reference
---

import manifest from "@/registry/manifest/tree-view";
...
Use `selectedValue` and `onSelectionChange` to control selection.
...
Use `canRename` and `onRenameComplete`. Press F2 on the focused node.
...
Use `asChild` on `TreeViewContent` with an `<a>` inside `TreeViewItem`.
...
Use `icon` and `expandedIcon` on `TreeViewBranchItem`. Pass `null` to hide.
...
Use `fileIcons` with `createFileIcons` to map extensions to icons.
...
<RTLComponentPreview componentName="tree-view" />
...
| Prop | Type | Default |
| --- | --- | --- |
| `--indentation` | `string` | `--spacing(4)` |
| `--item-gap` | `string` | `--spacing(2)` |
| `--padding-block` | `string` | `--spacing(1.5)` |
| `--padding-inline` | `string` | `--spacing(3)` |
| `--icon-size` | `string` | `--spacing(4)` |
...
Provides a TreeView API created with `useTreeView` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseTreeViewReturn` | `required` |
| `fileIcons` | `Record<string, React.JSX.ElementType \| null>` | `-` |

Pass the same `fileIcons` you would use on `TreeView` so leaf items resolve extension icons under the headless path.

### TreeViewBranchIndicator

Public TreeViewBranchIndicator component. It forwards the underlying primitive props.
