...
Render `DrawerSwipeArea` to open the drawer by swiping from its edge. The hit area follows the drawer direction and is inactive while the drawer is open.

<ComponentPreview componentName="drawer" fileName="example-swipe-area" />

### Scrollable

`DrawerBody` fills the remaining panel height and scrolls independently, keeping the header and footer visible.
...
Nested drawers scale, shift, and smoothly resize the previous drawer to the frontmost panel height. Swipe the top drawer slowly to see the previous layer restore as the gesture progresses.
...
| Prop | Type | Default |
| --- | --- | --- |
| `--bleed` | CSS custom property | `--spacing(12)` |
| `--space` | CSS custom property | `--spacing(6)` |
...
Public DrawerProvider component. It forwards the underlying primitive props.
