---
title: Listbox
description: Select one or many items from a list.
seo:
  description: Select one or many items from an inline list.
links:
  doc: https://ark-ui.com/docs/components/listbox
  api: https://ark-ui.com/docs/components/listbox#api-reference
---

import manifest from "@/registry/manifest/listbox";
...
<ComponentPreview componentName="listbox" fileName="example-vertical" />
...
Set `showIndicator={false}` on `ListboxItem` to hide the selected item checkmark.

<ComponentPreview componentName="listbox" fileName="example-without-indicator" />
...
Filter input for the listbox. Use with `useListCollection` and `onChange` to filter items.
...
Checkmark or icon shown when the item is selected. `ListboxItem` includes this by default; use it for custom indicator composition.
...
Provides a Listbox API created with `useListbox` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseListboxReturn` | **required** |

### ListboxItemDescription

Public ListboxItemDescription component. It forwards the underlying primitive props.
...
- [Tags Input](/docs/components/tags-input) - input that manages removable tags
