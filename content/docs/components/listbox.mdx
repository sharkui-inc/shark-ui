...
Use the listbox within a popover to create menu-like.

<ComponentPreview componentName="listbox" fileName="example-with-popover" />

### Grid Layout

Use `createGridCollection` with `columnCount` and wrap the items in a `div` with grid CSS.
`ListboxContent` owns the scrollable surface; use the wrapper for collection layout such as grid columns and gaps.
