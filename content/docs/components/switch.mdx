---
title: Switch
description: Toggle a binary on/off state.
seo:
  description: On/off switch for settings and feature toggles.
links:
  doc: https://ark-ui.com/docs/components/switch
  api: https://ark-ui.com/docs/components/switch#api-reference
---

import manifest from "@/registry/manifest/switch";

<ComponentPreview componentName="switch" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/switch
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/switch.tsx" src="/switch.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { Switch } from "@/components/ui/switch";
```

```tsx
<Switch />
```

## Controlled

Use the `checked` and `onCheckedChange` props to control the switch.

<ComponentPreview componentName="switch" fileName="example-controlled" />

## States

### Invalid 

Use the `invalid` prop on the `Field` or in the `Switch` component to mark the switch as invalid.

<ComponentPreview componentName="switch" fileName="example-invalid" />

## Examples

### Disabled

Use the `disabled` prop to disable the switch.

<ComponentPreview componentName="switch" fileName="example-disabled" />

### With Description

Use `FieldDescription` to add a description to the switch.

<ComponentPreview componentName="switch" fileName="example-description" />

### Card Style

Use a card-style layout with the switch for a more prominent display.

<ComponentPreview componentName="switch" fileName="example-card" />

### Form Integration

Integrate the switch into a form with proper validation and error handling.

<ComponentPreview componentName="switch" fileName="example-form" />

### Customizing Size

The switch size is controlled by the `[--size-*]` CSS variable.

<ComponentPreview componentName="switch" fileName="example-size" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="switch" />

## API Reference

### Switch

Toggle switch for on/off state. Use with a form control.

| Prop | Type | Default |
| --- | --- | --- |
| `checked` | `boolean` | `-` |
| `defaultChecked` | `boolean` | `false` |
| `disabled` | `boolean` | `false` |
| `invalid` | `boolean` | `false` |
| `name` | `string` | `-` |
| `value` | `string \| number` | `"on"` |
| `onCheckedChange` | `(details: CheckedChangeDetails) => void` | `-` |
| `className` | `string` | `-` |

| Prop | Type | Default |
| --- | --- | --- |
| `--size` | `string` | `--spacing(5)` |

### SwitchRootProvider

Provides a Switch API created with `useSwitch` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseSwitchReturn` | **required** |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/switch#api-reference).

## Related

- [Checkbox](/docs/components/checkbox) - multi-select checkbox
- [Toggle](/docs/components/toggle) - two-state toggle button
- [Radio Group](/docs/components/radio-group) - single-select radios
