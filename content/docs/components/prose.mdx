---
title: Prose
description: Typography for HTML content.
seo:
  description: Typography defaults for long-form HTML.
---

import manifest from "@/registry/manifest/prose";
...
.prose iframe:not(.not-prose iframe) {
  margin-top: calc(var(--spacing) * 6);
  margin-bottom: calc(var(--spacing) * 6);
}
...
<RTLComponentPreview componentName="prose" autoHeight />
...
Public Prose component. It forwards the underlying primitive props.
