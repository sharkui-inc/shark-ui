...
Fixed sidebars keep their content clear of the anchored side safe area. The mobile sidebar inherits the same behavior from [Sheet](/docs/components/sheet).

This takes effect when the consuming app enables `viewport-fit=cover`; browsers without a safe area keep the existing layout.
