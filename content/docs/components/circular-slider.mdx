---
title: Circular Slider
description: Circular slider for selecting values.
seo:
  description: Dial-style slider for compact numeric values.
links:
  doc: https://ark-ui.com/docs/components/angle-slider
  api: https://ark-ui.com/docs/components/angle-slider#api-reference
---

import manifest from "@/registry/manifest/circular-slider";

<ComponentPreview componentName="circular-slider" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```bash
npx shadcn@latest add @shark/circular-slider
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>This component depends on [Field](/docs/components/field). Install it first if you haven't already.</Step>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/circular-slider.tsx" src="/circular-slider.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { CircularSlider } from "@/components/ui/circular-slider";
```

```tsx
<CircularSlider defaultValue={45} />
```

## Controlled

<ComponentPreview componentName="circular-slider" fileName="example-controlled" />

## States

### Disabled

<ComponentPreview componentName="circular-slider" fileName="example-disabled" />

## Examples

### Size

Use the `size` prop to change the slider dimensions.

<ComponentPreview componentName="circular-slider" fileName="example-size" />

### Thickness

Use the `thickness` prop to change the progress ring thickness.

<ComponentPreview componentName="circular-slider" fileName="example-thickness" />

### With markers

Use `markers` to display clock-style tick marks.

<ComponentPreview componentName="circular-slider" fileName="example-with-markers" />

### With step

Use `markersAtSteps` with `step` to align markers with step positions.

<ComponentPreview componentName="circular-slider" fileName="example-step" />

### Show value

Use `CircularSliderValue` to show the current value. The `prefix` and `suffix` props customize the display.

<ComponentPreview componentName="circular-slider" fileName="example-with-value" />

### Custom markers

Pass a `number[]` to `markers` for custom angles.

<ComponentPreview componentName="circular-slider" fileName="example-custom-markers" />

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="circular-slider" />

## API Reference

### CircularSlider

Root component. Manages angle selection and value.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `number` | - |
| `defaultValue` | `number` | `0` |
| `onValueChange` | `(details: ValueChangeDetails) => void` | - |
| `step` | `number` | `1` |
| `disabled` | `boolean` | - |
| `size` | `number` | `100` |
| `thickness` | `number` | `6` |
| `markers` | `boolean \| number[]` | - |
| `markersAtSteps` | `boolean` | `false` |
| `className` | `string` | - |

### CircularSliderLabel

Accessible label. Render as a child of `CircularSlider`.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | - |

### CircularSliderValue

Shows the current angle value (degrees or custom format).

| Prop | Type | Default |
| --- | --- | --- |
| `prefix` | `React.ReactNode \| string` | - |
| `suffix` | `React.ReactNode \| string` | - |
| `className` | `string` | - |

### CircularSliderRootProvider

Provides a CircularSlider API created with `useCircularSlider` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseCircularSliderReturn` | **required** |
| `size` | `number` | `100` |
| `thickness` | `number` | `6` |

`size` and `thickness` must match the visual CSS on the consumer tree so Thumb/Marker metrics stay aligned.

### CircularSliderThumb

Public CircularSliderThumb component. It forwards the underlying primitive props.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | - |

### CircularSliderMarkerGroup

Public CircularSliderMarkerGroup component. It forwards the underlying primitive props.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | - |

### CircularSliderMarker

Public CircularSliderMarker component. It forwards the underlying primitive props.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `className` | `string` | - |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/react/docs/components/angle-slider).
