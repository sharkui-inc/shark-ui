---
title: Collapsible
description: Show or hide content behind a trigger.
seo:
  description: Show or hide a block of content behind a trigger.
links:
  doc: https://ark-ui.com/docs/components/collapsible
  api: https://ark-ui.com/docs/components/collapsible#api-reference
---

import manifest from "@/registry/manifest/collapsible";

<ComponentPreview componentName="collapsible" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark/collapsible
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

<RegistryDependencies dependencies={manifest.dependencies} />

<Step>Ensure your CSS entry imports `tw-animate-css`.</Step>

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/collapsible.tsx" src="/collapsible.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Anatomy

```text
Collapsible
├── CollapsibleTrigger
└── CollapsibleContent
```

## Usage

```tsx
import { 
  Collapsible, 
  CollapsibleTrigger, 
  CollapsibleContent 
} from "@/components/ui/collapsible";
```

```tsx
<Collapsible>
  <CollapsibleTrigger>Can I use this in my project?</CollapsibleTrigger>
  <CollapsibleContent>
    Yes. Free to use for personal and commercial projects. No attribution
    required.
  </CollapsibleContent>
</Collapsible>
```

## Controlled

Use the `open` and `onOpenChange` props to control the collapsible.

<ComponentPreview componentName="collapsible" fileName="example-controlled" />

## States

### Disabled

<ComponentPreview componentName="collapsible" fileName="example-disabled" />

## Examples

### Nested

Collapsibles can be nested to create hierarchical structures.

<ComponentPreview componentName="collapsible" fileName="example-nested" />

### Partial Collapse

Use the `collapsedHeight` or `collapsedWidth` prop to show a portion of the content when collapsed.

<ComponentPreview componentName="collapsible" fileName="example-partial-collapse" />


<Alert variant="warning">
  <InfoIcon />
  <AlertTitle>Accessibility Notice</AlertTitle>
  <AlertDescription>
  Interactive elements (links, buttons, inputs) within the collapsed area automatically become inaccesible to prevent keyboard navigation to hidden content.
  </AlertDescription>
</Alert>

## RTL

To enable RTL support, see the [RTL configuration guide](/docs/rtl).

<RTLComponentPreview componentName="collapsible" />

## API Reference

### Collapsible

Root component. Controls expand/collapse state and animation.

| Prop | Type | Default |
| --- | --- | --- |
| `open` | `boolean` | `-` |
| `defaultOpen` | `boolean` | `false` |
| `onOpenChange` | `({ open }: OpenChangeDetails) => void` | `-` |
| `collapsedHeight` | `string` | `-` |
| `collapsedWidth` | `string` | `-` |
| `lazyMount` | `boolean` | `true` |
| `unmountOnExit` | `boolean` | `true` |
| `hideMode` | `"display-none" \| "activity"` | `"display-none"` |
| `className` | `string` | `-` |

### CollapsibleTrigger

Toggles expand/collapse on click. Use `asChild` for custom trigger elements.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### CollapsibleContent

Holds the content that expands and collapses. Animated by default.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### CollapsibleIndicator

Chevron or icon that rotates to indicate open/closed state.

| Prop | Type | Default |
| --- | --- | --- |
| `className` | `string` | `-` |
| `asChild` | `boolean` | `false` |

### CollapsibleRootProvider

Provides a Collapsible API created with `useCollapsible` to descendant components.

| Prop | Type | Default |
| --- | --- | --- |
| `value` | `UseCollapsibleReturn` | **required** |

---

For a complete list of props, see the [Ark UI documentation](https://ark-ui.com/docs/components/collapsible#api-reference).

## Related

- [Accordion](/docs/components/accordion) - expandable stacked sections
- [Sidebar](/docs/components/sidebar) - responsive app sidebar
- [Tabs](/docs/components/tabs) - tabbed panel navigation
