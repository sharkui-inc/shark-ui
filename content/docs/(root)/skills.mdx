...
```bash
npx skills add vinihvc/shark-ui
```
...
Tailwind CSS v4, semantic tokens, and the small habits that keep Shark visually consistent (`data-slot` usage, spacing, borders), plus the editorial rules we keep for examples and previews (thumbs, field widths, chart tooltips, sidebar preview framing, and similar detail).

### Migration patterns

When someone pastes Radix- or shadcn-shaped code, the skill nudges the rewrite toward **Shark UI**: collection objects for list primitives, `asChild` where Shark UI documents it, and the chart or sidebar patterns we actually ship, not a generic “swap the import path” pass.
...
The skill is versioned in the same open-source project as Shark UI. If you want to read or change it, open the project on [GitHub](https://github.com/vinihvc/shark-ui).
