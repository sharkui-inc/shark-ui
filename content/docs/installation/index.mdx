...
With the `@shark` registry in `components.json` ([namespaced registries](https://ui.shadcn.com/docs/registry/namespace)), a single meta item pulls in the full catalog:

```bash
npx shadcn@latest add @shark/ui
```

That resolves [`/r/ui.json`](https://shark.vini.one/r/ui.json), which lists every other component as `registryDependencies`. You can still add pieces individually with `@shark/<name>` or the per-component URL.
